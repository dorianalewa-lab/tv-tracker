...
import { Trash2, MoreVertical, Bookmark } from 'lucide-react';
...
export function LibraryScreen() {
  ...
  const savedItems = useMemo(
    () => Object.values(db.items).filter((it) => it.saved).sort((a, b) => (a.updatedAt < b.updatedAt ? 1 : -1)),
    [db]
  );
      ...
      <div className="sticky top-0 z-10 bg-bg/95 backdrop-blur border-b border-border">
        ...
        <div className="px-4 pb-2">
          <div className="inline-flex rounded-full border border-border bg-surface p-0.5">
            {(['titles', 'saved'] as View[]).map((v) => (
              <button
                key={v}
                onClick={() => setView(v)}
                className={`px-4 py-1.5 rounded-full text-sm ${
                  view === v ? 'bg-accent text-black font-medium' : 'text-muted'
                }`}
              >
                {v === 'titles' ? 'Titres' : (
                  <span className="inline-flex items-center gap-1">
                    <Bookmark size={12} /> Enregistrés{savedItems.length ? ` · ${savedItems.length}` : ''}
                  </span>
                )}
              </button>
            ))}
      ...
      <div className="px-4 pt-4">
        {view === 'titles' ? (
          items.length === 0 ? (
            ...
            <ItemsGrid items={items} onMenu={setMenuFor} />
          )
        ) : savedItems.length === 0 ? (
          <div className="text-muted text-sm text-center py-16">
            Rien d'enregistré pour l'instant. Sur une fiche, tap l'icône 🔖 pour épingler un titre ici.
          </div>
        ) : (
          <ItemsGrid items={savedItems} onMenu={setMenuFor} />
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-end sm:items-center justify-center" onClick={() => setMenuFor(null)}>
          ...
          >
            ...
            <div className="px-4 py-2 text-xs text-muted">
              Statut auto : <span className="text-text">{menuFor.status === 'watching' ? 'En cours' : menuFor.status === 'completed' ? 'Terminé' : 'À voir'}</span>
            </div>
            <button
              onClick={() => {
              ...
              className="w-full flex items-center gap-3 px-4 py-3.5 text-left text-red-400 hover:bg-bg border-t border-border"
            >
              <Trash2 size={18} />
...
function ItemsGrid({ items, onMenu }: { items: LibraryItem[]; onMenu: (it: LibraryItem) => void }) {
  return (
